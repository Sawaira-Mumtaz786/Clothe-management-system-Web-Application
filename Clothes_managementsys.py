...
# Import our AI Helper module
try:
    from ai_helper import analyze_clothing_image
except ImportError:
    analyze_clothing_image = None
...
def generate_report(interval='daily', start_date=None, end_date=None):
    ...
    for row in rows:
        ...
        if key not in per_item:
            per_item[key] = {
                "purchased": 0, "purchase_total": 0.0,
                "sold": 0, "sale_total": 0.0, "cogs": 0.0
            }
    ...
    conn.close()
    return {
        "transactions": rows, "total_sales": total_sales,
        "total_purchases": total_purchases, "total_COGS": total_COGS,
        "profit": profit, "qty_purchased": qty_purchased,
        "qty_sold": qty_sold, "per_item": per_item,
    }
...
def export_report_csv(report_dict, filename):
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        ...
        w.writerow(["Item", "Purchased Qty", "Purchased Total", "Sold Qty", "Sale Total", "COGS", "Profit"])
        for item, vals in report_dict.get('per_item',{}).items():
            profit_item = vals.get('sale_total',0) - vals.get('cogs',0)
            w.writerow([
                item, vals.get('purchased',0), f"${vals.get('purchase_total',0):.2f}",
                vals.get('sold',0), f"${vals.get('sale_total',0):.2f}",
                f"${vals.get('cogs',0):.2f}", f"${profit_item:.2f}"
            ])
...
def export_stock_csv(filename):
    ...
    with open(filename,'w',newline='',encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(["Clothes Management System – Stock Report", datetime.now().strftime("%Y-%m-%d %H:%M")])
        w.writerow([])
        w.writerow(["Code","Name","Category","Season","Size","Color","Qty","Cost Price","Sale Price","Status"])
        for r in rows:
...
def build_report_text(report_dict, title="Report"):
    lines = [
        "=" * 70, f"  CLOTHES MANAGEMENT SYSTEM – {title.upper()}",
    ...
    for item, vals in report_dict.get('per_item',{}).items():
        profit_item = vals.get('sale_total',0) - vals.get('cogs',0)
        lines.append(
            f"  {item:<35} {vals.get('sold',0):>6} ${vals.get('sale_total',0):>9.2f} "
            f"{vals.get('purchased',0):>6} ${vals.get('purchase_total',0):>9.2f} "
            f"${vals.get('cogs',0):>9.2f} ${profit_item:>9.2f}"
        )
    lines += ["", "=" * 70]
    return "\n".join(lines)
...
def build_stock_report_text():
    rows = get_stock_summary()
    lines = [
        "=" * 70, "  CLOTHES MANAGEMENT SYSTEM – CURRENT STOCK REPORT",
        f"  Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", "=" * 70, "",
        f"  {'Code':<12} {'Name':<20} {'Cat':<8} {'Size':<5} {'Color':<10} {'Qty':>5}  Status", "-" * 70,
    ]
    total = 0
    for r in rows:
...
class StyledButton(tk.Button):
    def __init__(self, parent, **kwargs):
        ...
        self.configure(bg=BUTTON_BG,fg=BUTTON_FG,relief='raised',padx=10,pady=5,font=('Arial',10))
...
class StyledFrame(tk.Frame):
    def __init__(self, parent, **kwargs):
        super().__init__(parent, **kwargs); self.configure(bg=FRAME_BG)
...
class ClothesManagementSystem(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Clothes Management System (AI Enabled)")
    ...
    def build_ui(self):
        ...
        self.alert_label = tk.Label(top_bar, text="", bg=WARNING_BG, fg="white", font=("Arial",10,"bold"), padx=8, pady=2)
        self.alert_label.pack(side='right', padx=10)

        self.user_label = tk.Label(top_bar, text="Not logged in", bg=HEADER_BG, fg=HEADER_FG, font=("Arial",10))
        self.user_label.pack(side='right', padx=20)

        tk.Button(top_bar, text="Logout", command=self.logout, bg=DANGER_BG, fg="white", relief='flat').pack(side='right', padx=5)
        tk.Button(top_bar, text="Change Password", command=self.change_password, bg=BUTTON_BG, fg="white", relief='flat').pack(side='right', padx=5)
    ...
    def create_login_tab(self):
        ...
        SuccessButton(login_frame, text="Login", command=self.login).grid(row=2,column=0,columnspan=2,pady=10,padx=10,sticky='we')
        return tab
    ...
    def create_reports_tab(self):
        ...
        self.report_period = ttk.Combobox(ctrl, values=["Daily","Weekly","Monthly","Yearly","Custom"],state="readonly",width=10)
        ...
        self.start_date = tk.Entry(ctrl,width=12,font=('Arial',10), state='disabled',disabledbackground="#e0e0e0", disabledforeground="#aaaaaa")
        self.start_date.pack(side='left')
        tk.Label(ctrl,text="To:",bg=FRAME_BG).pack(side='left',padx=(10,5))
        self.end_date = tk.Entry(ctrl,width=12,font=('Arial',10), state='disabled',disabledbackground="#e0e0e0", disabledforeground="#aaaaaa")
    ...
    def create_users_tab(self):
        ...
        StyledButton(reg_frame,text="Create User",command=self.register_user).grid(row=len(labels),column=0,columnspan=2,pady=10,padx=10,sticky='we')
    ...
    def apply_permissions(self):
        ...
        self.add_btn.config(state='normal' if is_admin else 'disabled')
        self.edit_btn.config(state='normal' if is_admin else 'disabled')
        self.delete_btn.config(state='normal' if is_admin else 'disabled')
        self.purchase_btn.config(state='normal' if is_admin else 'disabled')
        self.sale_btn.config(state='normal' if self.active_role in ['admin','staff'] else 'disabled')
    ...
    def load_items(self, search_term=None, category=None, season=None, color=None, size=None):
    ...
    def delete_item(self):
        ...
        if messagebox.askyesno("Confirm Delete", f"Delete '{self.selected_item['name']}'?", icon='warning'):
    ...
    def on_report_period_change(self, event=None):
        ...
        if period == "Custom":
            ...
            for entry in (self.start_date, self.end_date):
                entry.config(state='normal'); entry.delete(0, tk.END); entry.config(state='disabled')
    ...
    def _get_report(self):
        ...
        return generate_report(period, start_date, end_date)
    ...
    def generate_report(self):
        ...
        summary = (f"📊 Report Summary\n"
                f"Total Sales: ${report['total_sales']:.2f} | Gross Profit: ${report['profit']:.2f}\n")
        ...
        for txn in report['transactions']:
            ...
            self.report_tree.insert('','end', values=(
                ds, txn[4].capitalize(), txn[3], txn[5], f"${txn[6]:.2f}", f"${txn[7]:.2f}", txn[9]
            ))
    ...
    def export_report(self):
        ...
        if fn: export_report_csv(report, fn)

    def print_report(self):
        report = self._get_report()
        if not report: return
        text = build_report_text(report, "Report")
        print_text_report(text)

    def export_stock_report(self):
        fn = filedialog.asksaveasfilename(defaultextension=".csv")
        if fn: export_stock_csv(fn)
...
class ItemDialog(tk.Toplevel):
    def __init__(self, parent, title, item_data=None):
        ...
        self.title(title); self.geometry("520x620")
    ...
    def create_widgets(self):
        ...
        for i,(label,wtype,opts) in enumerate(rows):
            ...
            if wtype == 'entry':
                ...
                w = tk.Entry(mf,width=30,font=('Arial',10),state='readonly',readonlybackground="#e8f8e8",fg=SUCCESS_BG)
                w.grid(row=i,column=1,pady=4,padx=(10,0),sticky='we')
            elif wtype == 'combo':
                ...
                tk.Entry(frm,textvariable=self.image_path,width=14,font=('Arial',9)).pack(side='left')
                StyledButton(frm,text="Browse",command=self.browse_image).pack(side='left',padx=2)
                
                # ✨ NEW: AI Auto-fill button!
                SuccessButton(frm,text="✨ AI Fill",command=self.run_ai_autofill).pack(side='left',padx=2)
                w = self.image_path
            self.fields[label] = w
    ...
    def run_ai_autofill(self):
        path = self.image_path.get()
        if not path or not os.path.exists(path):
            messagebox.showwarning("Select Image", "Please select an image first.")
            return

        if not analyze_clothing_image:
            messagebox.showerror("Error", "AI module not available.")
            return

        data = analyze_clothing_image(path)
        if data:
            if "name" in data:
                self.fields["Item Name:"].delete(0, tk.END)
                self.fields["Item Name:"].insert(0, data["name"])
            if "category" in data and data["category"] in ["Men","Women","Kids"]:
                self.fields["Category:"].set(data["category"])
            if "season" in data and data["season"] in ["Summer","Winter"]:
                self.fields["Season:"].set(data["season"])
            if "size" in data:
                self.fields["Size:"].set(data["size"])
            if "color" in data:
                self.fields["Color:"].delete(0, tk.END)
                self.fields["Color:"].insert(0, data["color"])
            messagebox.showinfo("AI Success", "Clothing details auto-filled using Gemini AI!")
        else:
            messagebox.showwarning("AI Notice", "Could not analyze image. Make sure GEMINI_API_KEY is set in your environment.")
    ...
    def save(self):
        ...
        if self.item_data:
            ok, msg = update_item(self.item_data['id'],name,category,season,size,color,quantity,pp,sp,image_path)
        else:
...
class TransactionDialog(tk.Toplevel):
    ...
    def create_widgets(self):
        mf = StyledFrame(self); mf.pack(fill='both',expand=True,padx=20,pady=20)
        tk.Label(mf,text=f"Record {self.txn_type.capitalize()}",font=("Helvetica",14,"bold"),bg=FRAME_BG).pack(pady=(0,12))

        inf = StyledFrame(mf); inf.pack(fill='x',pady=8)
        tk.Label(inf,text=f"Item: {self.item_data['name']}",font=("Helvetica",12,"bold"),bg=FRAME_BG).pack(anchor='w')
        ...
        default_price = (self.item_data['purchase_price'] if self.txn_type == 'purchase' else self.item_data['sale_price'])
        ...
        tk.Button(bf,text=f"Record {self.txn_type.capitalize()}",command=self.record,bg=color_bg,fg="white").pack(side='left',padx=10)
        tk.Button(bf,text="Cancel",command=self.destroy,bg="#95a5a6",fg="white").pack(side='left',padx=10)
    ...
    def record(self):
        try:
            ...
            messagebox.showerror("Input Error","Invalid input"); return
        ok, msg = record_transaction(self.item_data['id'],self.txn_type,qty,price,self.parent.active_user)
        if ok: messagebox.showinfo("Success",msg); self.result = True; self.destroy()
...
class ChangePasswordDialog(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        tk.Label(mf,text="Old Password:",bg=FRAME_BG).pack(anchor='w')
        self.old_pw = tk.Entry(mf,show="*",width=28); self.old_pw.pack(fill='x',pady=4)
        tk.Label(mf,text="New Password:",bg=FRAME_BG).pack(anchor='w')
    ...
    def change_password(self):
        ...
        if ok: messagebox.showinfo("Success",msg); self.destroy()
